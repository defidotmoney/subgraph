...
function getOrCreateMoneyDebt(timestamp: BigInt): MoneyDebt {
  ...
  let moneyDebtId = `money_debt_${dateDayString}`
  let moneyDebt = MoneyDebt.load(moneyDebtId)
  
  if (!moneyDebt) {
    moneyDebt = new MoneyDebt(moneyDebtId)
    moneyDebt.dateDay = dateDayString
...
function getOrCreateLoanCount(): LoanCount {
  let loanCountId = "loan_count"
  let loanCount = LoanCount.load(loanCountId)
  
  if (!loanCount) {
    loanCount = new LoanCount(loanCountId)
    loanCount.lifetime = BigInt.fromI32(0)
    loanCount.current = BigInt.fromI32(0)
  }
...
export function handleCreateLoan(event: CreateLoanEvent): void {
  ...
  // Update LoanCount
  let loanCount = getOrCreateLoanCount()
  loanCount.lifetime = loanCount.lifetime.plus(BigInt.fromI32(1))
  loanCount.current = loanCount.current.plus(BigInt.fromI32(1))
  loanCount.save()

  let historyItem = new HistoryItem(
    event.transaction.hash.concatI32(event.logIndex.toI32())
  )
  historyItem.type = "create-loan"
  historyItem.user = event.params.account
  historyItem.contract_address = event.address
  historyItem.market = event.params.market
  historyItem.collateralAmount = event.params.coll_amount
  historyItem.debtAmount = event.params.debt_amount
...
export function handleAdjustLoan(event: AdjustLoanEvent): void {
  ...
  historyItem.type = "adjust-loan"
  historyItem.user = event.params.account
  historyItem.contract_address = event.address
  historyItem.market = event.params.market
  historyItem.collateralAmount = event.params.coll_adjustment
  historyItem.debtAmount = event.params.debt_adjustment
  historyItem.blockNumber = event.block.number
  historyItem.blockTimestamp = event.block.timestamp
  historyItem.transactionHash = event.transaction.hash
  historyItem.save()
...
export function handleCloseLoan(event: CloseLoanEvent): void {
  ...
  // Update LoanCount
  let loanCount = getOrCreateLoanCount()
  loanCount.current = loanCount.current.minus(BigInt.fromI32(1))
  loanCount.save()

  let historyItem = new HistoryItem(
    event.transaction.hash.concatI32(event.logIndex.toI32())
  )
  historyItem.type = "close-loan"
  historyItem.user = event.params.account
  historyItem.contract_address = event.address
  historyItem.market = event.params.market
  historyItem.collateralAmount = event.params.coll_withdrawn
  historyItem.debtAmount = event.params.debt_repaid
  historyItem.blockNumber = event.block.number
  historyItem.blockTimestamp = event.block.timestamp
  historyItem.transactionHash = event.transaction.hash
  historyItem.save()
...
export function handleLiquidateLoan(event: LiquidateLoanEvent): void {
  ...
  historyItem.type = "liquidate-loan"
  historyItem.user = event.params.account
  historyItem.contract_address = event.address
  historyItem.market = event.params.market
  historyItem.collateralAmount = event.params.coll_received
  historyItem.debtAmount = event.params.debt_repaid
  historyItem.liquidator = event.params.liquidator
  historyItem.blockNumber = event.block.number
  historyItem.blockTimestamp = event.block.timestamp
  historyItem.transactionHash = event.transaction.hash
  historyItem.save()
